...
opti.minimize(theta_2(1) * vars.costs.joint_vel_cost + theta_2(2) * vars.costs.joint_torque_cost + theta_2(3) * vars.costs.ee_vel_cost);
sol_2 = opti.solve();
lambda_2 = sol_2.value(opti.lam_g); % Extract dual variables
% Extract primal variables
q_2 = sol_2.value(vars.variables.q);
dq_2 = sol_2.value(vars.variables.dq);
ddq_2 = sol_2.value(vars.variables.ddq);
...
opti_ioc.subject_to(vec(vars_ioc.variables.q - q_1) <= pi/4)
opti_ioc.subject_to(vec(-vars_ioc.variables.q + q_1) <= pi/4)
...
% Instantiate
instantiate_ndof_model(vars_ioc, opti_ioc, dt, q0, dq0, L, COM, M, I, gravity, Fext, goal, ddq_2, dq_2, q_2);
